...
(function($){
	$.lrc = {
		...
		start: function(txt, callback) {
			...
			if(this.list.length > 0) {
				...
				for(var i = 0; i < this.list.length; i++)
					...
					html += '<li data-time="'+this.list[i][0]+'">'+this.list[i][1]+'</li>';
		...
		jump: function(duration) {
			...
			this.pivot = pivot;
			this.currentLrc = $('#'+this.prefixid+'_list').children().eq(this.pivot).text();
